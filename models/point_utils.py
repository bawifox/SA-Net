...
def _knn_torch(query, database, k, return_dist=False):
    """
    PyTorch 版本的 kNN（不依赖 KeOps），作为降级或强制替代实现。
    """
    with torch.no_grad():
        ...
        # torch.cdist 支持 batched [B, N_q, C] x [B, N_d, C] -> [B, N_q, N_d]
        dist = torch.cdist(query, database, p=2)  # [B, N_q, N_d]
        nn_dist, nn_idx = torch.topk(dist, k, dim=2, largest=False, sorted=True)  # [B, N_q, K]
...
def knn_keops(query, database, k, return_dist=False):
    ...
    """
    # 环境变量可以强制使用纯 PyTorch 实现，完全绕开 KeOps
    if os.environ.get("AFF_USE_TORCH_KNN", "0") == "1":
        return _knn_torch(query, database, k, return_dist=return_dist)

    try:
        from pykeops.torch import LazyTensor  # type: ignore
    except Exception:
        # KeOps 未安装或导入失败，退回到 PyTorch 实现
        return _knn_torch(query, database, k, return_dist=return_dist)
    ...
    try:
        ...
        # KeOps 在运行时出错时静默退回 PyTorch 实现，避免日志噪声
        return _knn_torch(query, database, k, return_dist=return_dist)
...
def space_filling_cluster(pos, m, h, w, no_reorder=False, sf_type='', use_anchor=True):
    ...
    with torch.no_grad():
        ...
        if use_anchor:
            ...
            # 数值稳定：防止浮点误差导致索引越界
            mean_assignment = mean_assignment.long().clamp_(min=0, max=order_grid_idx.numel() - 1)
            mean_assignment = (
                order_grid_idx.unsqueeze(0)
                .expand(b, -1)
                .gather(index=mean_assignment, dim=1)
                .unsqueeze(2)
            )  # b x n x 1
