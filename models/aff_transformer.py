...
from .point_utils import knn_keops, space_filling_cluster, points2img
...
class ClusterMerging(nn.Module):
    ...
    def forward(self, pos, feat, member_idx, cluster_mask, learned_prob, stride, pe_idx, reserve_num):
        ...
        if self.reserve_on:
            # Robustly construct reserve indices per batch to avoid shape mismatch
            reserve_indices = []
            for bi in range(b):
                idx_b = reserve_mask[bi].nonzero(as_tuple=True)[0]  # 1D indices
                if idx_b.numel() >= reserve_num:
                    idx_b = idx_b[:reserve_num]
                else:
                    # 如果实际保留点少于 reserve_num,则重复填充
                    if idx_b.numel() == 0:
                        # 极端情况下没有保留点,则从所有 token 中取前 reserve_num 个
                        idx_b = torch.arange(reserve_num, device=pos.device)
                    else:
                        repeat_factor = (reserve_num + idx_b.numel() - 1) // idx_b.numel()
                        idx_b = idx_b.repeat(repeat_factor)[:reserve_num]
                reserve_indices.append(idx_b)
            reserve_idx = torch.stack(reserve_indices, dim=0)  # b x reserve_num
...
class AutoFocusFormer(nn.Module):
    ...
    def forward_backbone(self, x, return_multi_scale: bool = False):
        """
        通用 backbone 前向接口:
        - return_multi_scale=False: 返回最后一层特征图 [B, C, H', W']
        - return_multi_scale=True: 返回多尺度特征列表 [(feat, H', W'), ...]
        注意:AFF 的空间尺寸通过 pos 坐标表示,这里统一用初始 patch 网格大小 (h, w)
        将 token 特征 scatter 回规则网格.
        """
        pos, tokens, h, w = self.patch_embed(x)  # tokens: [B, N, C0]
        ...
        for i_layer, layer in enumerate(self.layers):
            ...
            if return_multi_scale:
                feat_grid = points2img(pos, tokens, base_h, base_w)  # [B, C, H, W]
                multi_scale.append((feat_grid, base_h, base_w))

        tokens = self.norm(tokens)  # [B, N, C]
        feat = points2img(pos, tokens, base_h, base_w)  # [B, C, H, W]
        if return_multi_scale:
            return multi_scale
        return feat, base_h, base_w

    def forward_features(self, x):
        """
        原分类用接口:返回 [B, C] 的全局 pooled 特征
        """
        feat, _, _ = self.forward_backbone(x, return_multi_scale=False)
        feat = feat.flatten(2).mean(dim=-1)  # 全局平均池化
        return feat
